...
export function SiteLocation({
  ...
  const aerialSrc = `/api/geo/static?type=aerial&lat=${lat}&lng=${lng}`
  const streetSrc = `/api/geo/static?type=streetview&lat=${lat}&lng=${lng}&heading=${heading}`

  // Click on the aerial recentres the pin on that spot.
  const onAerialClick = (e: React.MouseEvent<HTMLImageElement>) => {
    ...
    const next = unproject(center.x + offsetX, center.y + offsetY)
    setLat(next.lat)
    setLng(next.lng)
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <div className="text-xs font-medium text-gray-500 mb-1">Aerial — click to reposition</div>

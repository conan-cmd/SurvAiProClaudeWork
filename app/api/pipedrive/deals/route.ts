...
import { pipedriveConfigured, searchDeals, PIPEDRIVE_SELECT } from "@/lib/pipedrive"

// Searches the firm's Pipedrive deals so a proposal can be linked to one.
export async function GET(request: NextRequest) {
  ...
  try {
    return NextResponse.json(await searchDeals(org, q))
  } catch (err) {
